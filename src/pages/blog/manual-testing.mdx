...
Assistive technologies, such as screen readers, use a browser's accessibility tree as the API to understand and interact with a site. For each node in the DOM, the browser determines if the node is semantically "interesting" and if so, adds it to the accessibility tree. There are over 100 HTML elements that come with built-in semantic properties. These include:
...
- **Name** - an element's accessible name can be derived from its content, an attribute such as `alt`, `aria-label`, `aria-labelledby`, `title`, or an associated element eg. an input's `label`.

`div` and `span` are the only HTML elements that have no semantic properties and are therefore be omitted from the tree. These should be used only as  wrappers for styling or to build custom components with roles that cannot be replaced by a native HTML element.
