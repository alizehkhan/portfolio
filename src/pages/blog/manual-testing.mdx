...
Since `div` and `span` are the only HTML elements that have no semantic properties, they are omitted from the tree. They should therefore either be used wrappers for styling or to build custom components with roles that cannot be replaced by a native HTML element.
