...
_There isn't much customisation when it comes to my ESLint config. I try to stick to the recommended standard for an accessible TypeScript React app._
...
ESLint needs to know which file extensions to consider when resolving imports in your code. Since I'm using TypeScript, I override the default config for `import/resolver`, adding `.ts` and `.tsx` to `extensions`.
