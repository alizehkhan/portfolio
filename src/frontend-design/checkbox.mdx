...
_I've been wanting to create a blog series for a while now on how to build common design system components in React that are accessible and follow best practice component API design. I've kept these unstyled to focus on markup and any state that may be required._
...
3. Use the native `InputHTMLAttributes`, omitting the `type` attribute, since this should remain `"checkbox"`. This includes all the event handlers and aria attributes. Use React's `children` prop for the label's content.
   ...
   <Banner>
   	Leverage existing and familiar APIs such HTML attributes and React's
   	children prop where possible instead of custom prop names. This reduces
...
```tsx
import { useId, type InputHTMLAttributes, forwardRef, Ref } from 'react'
	...
	(props: CheckboxProps, ref?: Ref<HTMLInputElement>) => {
		...
		return (
			<label htmlFor={id} className={className}>
				<input {...restProps} ref={ref} type="checkbox" id={id} />
				{children}
			</label>
		)
	},
...
1. Create a `CheckboxGroup` component that will wrap our checkboxes in a `<fieldset>` with a `<legend>` for the title.
   ...
   <Banner>
   	Building complex components in a composable way as opposed to monolothic
   	with several props will allow users to drop a level below the higher
   	abstraction.
   </Banner>

2. Below the `title` and above the checkboxes we will add `<p>` elements for `hint` and `error` texts to render when needed.
   <Banner>
   	Avoid placing hints and error messages below the input because browser's
   	auto-complete features and on-screen keyboards can obscure them. It's also
   	an anti-pattern because the texts would be announced by screen readers
   	after the input.
   </Banner>
   So that the screen reader knows these elements provide more information for
   the `fieldset` we list the `id`s of these elements in an `aria-describedby`
   attached to the `fieldset`.
