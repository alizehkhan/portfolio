...
2. Fix all auto-fixable ESLint problems on save. To do this, open the command palette, <kbd>command</kbd>+<kbd>shift</kbd>+<kbd>P</kbd>, search `Preferences: Open User Settings (JSON)` and add the following configuration to your `settings.json`:
...
<div className="my-4">
	- I add `@typescript-eslint/parser` to parse TypeScript code into
	ESLint-compatible nodes. - I add the recommended configuration from
