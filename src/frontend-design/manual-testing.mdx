...
_Tackling accessibility can feel overwhelming and it's pretty much impossible to have a 100% accessible website. So instead of creating a guide that tries to boil the ocean, I thought I would share the manual testing rituals I've managed to incorporate into my development workflow. I'll try to keep this up to date as I learn more._

_If you've never used the screen reader on your Mac, my hope is that by the end of this post you will have at least pressed <kbd>command</kbd> + <kbd>F5</kbd>. I mean you could even press it now if you want 🤷._
...
<kbd>tab</kbd> to focus on the next interactive element.

<kbd>shift</kbd> + <kbd>tab</kbd> to focus on the previous interactive element.

<kbd>↑</kbd>/<kbd>↓</kbd>/<kbd>←</kbd>/<kbd>→</kbd> to cycle through related
controls such a dropdown list or radio group.

<kbd>spacebar</kbd> to toggle states (eg. for a checkbox) or move down the page

<kbd>enter</kbd> to interact.
...
<kbd>command</kbd> + <kbd>F5</kbd> to activate MacOS's built-in screen reader,
Voiceover.

<kbd>control</kbd> + <kbd>option</kbd> + <kbd>←</kbd>/<kbd>→</kbd> to move
between elements

<kbd>control</kbd> + <kbd>option</kbd> + <kbd>U</kbd> to open up the rotor. Then
<kbd>←</kbd>/<kbd>→</kbd> to navigate through the lists available eg. landmarks,
headers, links etc.
...
- Open up the screen reader rotor <kbd>control</kbd> + <kbd>option</kbd> + <kbd>U</kbd> and check the headings list for the page. Are there any headings missing or heading tags being misused?
...
<kbd>tab</kbd> through the page and check:
